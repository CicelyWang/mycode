...
def get_label(data):
    ...
    if os.path.exists(result_path):
        ...
        true = dict(zip(DATA['orderid'].values, DATA
        ['geohashed_end_loc']))
        pickle.dump(true, open(result_path, 'wb+'))
...
def get_loc_to_loc(train,test):
    ...
    if os.path.exists(result_path) & flag:
        ...
        #sloc_eloc_count_se = train.pivot_table(['geohashed_start_loc','geohashed_end_loc'],values='orderid',aggfunc='count')
...
def get_user_count(train,result):
    ...
    user_count_se = train.pivot_table(index='userid',values='orderid',aggfunc='count')
    user_count = pd.DataFrame({'userid':user_count_se.index,'user_count':user_count_se.values})
...
if __name__ == "__main__":
    ...
    train1 = train
    train2 = train[(train['starttime'] > '2017-05-20 00:00:00')]
    ...
    result.fillna('0',inplace=True)
    result.to_csv('result_0517.csv',index=False,header=False)
